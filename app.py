        ...
        try:
            ...
            try:
                ...
                if os.path.exists(CREDENTIALS_PATH):
                    ...
                    try:
                        # Make sure Google credentials are set properly
                        os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = CREDENTIALS_PATH
                        ...
                        # Check if the predictor is actually available
                        if not predictor.is_available:
                            print("Predictor reports it is not available, falling back to random predictions")
                        ...
                        # Verify predictions were actually added - check for both possible keys
                        has_predictions = False
                        if enhanced_routes and len(enhanced_routes) > 0:
                            for route in enhanced_routes:
                                for segment in route.get('segments', []):
                                    # Check for either delay_prediction or predicted_delay
                                    if 'delay_prediction' in segment:
                                        has_predictions = True
                                        print(f"Found prediction: {segment.get('delay_prediction')}")
                                        break
                                if has_predictions:
                                    break
                        ...
                        if has_predictions:
                            ...
                            # Dump a sample segment to debug
                            if enhanced_routes and enhanced_routes[0]['segments']:
                                sample = enhanced_routes[0]['segments'][0]
                                print(f"Sample segment data: {sample}")
            ...
            if filtered_routes and len(filtered_routes) > 0:
                ...
                if 'delay_prediction' in sample_segment:
                    delay_value = sample_segment['delay_prediction'].get('predicted_delay', 'No delay value')
                    print(f"Sample segment delay: {delay_value}")
                else:
                    print("Sample segment delay: No delay prediction found")
